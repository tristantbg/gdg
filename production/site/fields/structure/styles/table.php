    ...
    <tr id="structure-entry-<?php echo $entry->id() ?>">
      ...
      <td>
        ...
        <?php endif ?>
          ...
          <?php if ($f['query']['value'] == '{{autoid}}'): ?>
          	<?php if ($foundPage = $index->findBy('autoid', $entry->{$f['name']})): ?>
          		<?php echo html('<strong>'.$foundPage->title().'</strong> ('.$foundPage->uri().')') ?>
          	<?php else: ?>
          		<?php echo 'La page a été supprimée' ?>
          	<?php endif ?>
          <?php else: ?>
